...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof (userInfo) == 'object' && userInfo.isAdmin && apiToken != undefined) {
      getJson(`http://localhost:8000/api/vendor/${userId}/all`, apiToken).then(setVendors)
    }
  }, [userInfo])
  ...
  const [activeTab, setActiveTab] = useState<"none" | "users" | "vendors">("none");
  ...
  const toggleVisibility = (type: 'vendor' | 'user', index: number) => {
    if (type == 'vendor') {
      ...
      patchJson(`http://localhost:8000/api/vendor/${vendors[index]._id}/hide`, {}, apiToken)
    }
    else {
            ...
            {users.map((user, idx:number) => (
              <li
                ...
                style={{
                  ...
                  boxShadow: user.hidden ? "inset 0 0 4px rgba(0,0,0,0.2)" : "none",
                }}
              >
                <span
            ...
            {vendors?.map((vendor: any, index: number) => (
              <li
                ...
                style={{
                  ...
                  boxShadow: vendor.hidden ? "inset 0 0 4px rgba(0,0,0,0.2)" : "none",
                }}
              >
                <span
